...
- **Downloads ファイル名規則（機能A）**: 推奨は `speech-tap-YYYYMMDD-HHmmss.txt`。
  既存ファイル衝突時の挙動（同秒内に連続停止した場合の連番付与か上書き禁止エラーか）を /architect で決定する。
- **「セッション境界」概念の実装方針（機能A）**: 「停止 → 次の開始まで」を 1 セッションとする粒度で良いか、
  あるいは `TranscriptStore` 側にセッション単位の確定列バッファを追加して停止時にスナップショット書き出しするか等、データ構造を /architect で確定。
- **翻訳のタイミング（機能B）**: finalized のみ翻訳するか、volatile も翻訳するかを決める（性能・体感遅延・翻訳料金等のトレードオフ。Apple Translation は無料だが処理量は変わる）。
- **翻訳パック未インストール時の UI フロー（機能B）**: ダイアログで誘導するか、メニュー / 状態行に通知するか、無音 fallback にメッセージ付与するか等を /architect で決定。
- **`Translator` / `LanguageDetector` の正確な API シグネチャ（機能B）**: 仕様で擬似シグネチャは示すが、
  実 Apple Translation framework / NLLanguageRecognizer の挙動に合わせて domain 中立な protocol を確定する。OS 型（`Translation.Session` / `NLLanguageRecognizer` 等）を domain に漏らさない境界設計を /architect で確定。
- **Composition Root への注入順序（機能A/B/C）**: 既存の `AppDelegate`（Composition Root）に新 port（`Translator` / `LanguageDetector` / `SessionExporter` 等）の生成・注入をどの順で組み込むか。既存の `TranscriptionService` のコンストラクタ拡張か、別の「表示パイプライン」を導入するか等を /architect で確定。
- **ADR-5 / ADR-6 の追記（/architect）**: ADR-5「翻訳エンジンに Apple Translation framework を採用し、画面表示と保存の経路を分離する」、ADR-6「セッション境界と Downloads エクスポート方針（ファイル名規則・失敗時挙動・非破壊性）」を /architect が記録する想定。
...
[infrastructure]
      ...
      AppleTranslator ─────────▶ Apple Translation framework（オンデバイス）
      AppleLanguageDetector ──▶ NaturalLanguage / NLLanguageRecognizer
      DownloadsSessionExporter ▶ ファイルシステム（~/Downloads）
```
...
- **翻訳エンジンの仕様変更**（言語パック未導入時の API 挙動等）は `AppleTranslator` に閉じる。`Translator` port シグネチャ不変なら domain への波及なし。
- **Downloads ファイル名規則の変更**（衝突時の挙動・タイムスタンプ書式）は `DownloadsSessionExporter` に閉じる。
- **ピン UI の振る舞い変更**は `TranscriptWindow`（presentation）に閉じる。
...
シグネチャの最終形は /architect が ADR-5/6 として確定する。ここでは「契約として満たすべき意味」を明文化する。

- **`Translator.translate(text, from, to) async throws -> String`（機能B）**:
  「原文 text を **オンデバイスで** sourceLocale から targetLocale へ翻訳する」操作と定義する。
  - **外部送信禁止**: 実装は外部ネットワークに音声・テキストを送信してはならない（固定要件）。
  - **不可時の throw 契約**: 翻訳パック未インストール / 利用不可な場合は明確な error を throw する（黙って空文字列・原文返却にしない）。呼び出し側が「原文フォールバック + ユーザー通知」を判断する。
  - **`Translator.ensureAvailable(for: Locale) async throws`**（任意・推奨）: 言語パックの利用可否を事前に確認する操作。未許諾なら初回ダウンロード許諾フローを起動する想定。
- **`LanguageDetector.detect(text) -> Locale?`（機能B）**:
  「テキストの言語を判定する」操作と定義する。
  - **判定不能時は `nil` を返す**（例: 空文字列・極端に短い文字列）。呼び出し側は「日本語ではない」とは扱わず、原文表示にフォールバックして良い。
  - **OS 型を漏らさない**: `NLLanguageRecognizer` / `NLLanguage` は port シグネチャに出さず、`Locale`（Foundation）を返す。
- **`SessionExporter.exportSession(segments, sessionId) async throws -> URL`（機能A・擬似シグネチャ）**:
  「セッション分の確定テキスト列を、独立した 1 ファイルとして指定の場所に書き出す」操作と定義する。
  - **戻り値**: 書き出した先の URL（UI への提示・後続検証に使う）。
  - **保存内容は原文のみ**（機能B の翻訳結果を含めない）。固定要件「画面表示と永続保存の経路分離」をここでも担保する。
  - **既存 `transcript.txt` の append 経路には触れない**（停止フローの主経路 finalize → flush は維持。Exporter は副経路）。
  - **失敗時の意味**: error を throw する。停止フロー全体は巻き戻さず、呼び出し側（StopFlowCoordinator）が「メイン保存は完了済み」を前提にユーザー通知する。
  - **`segments` の境界**: 「セッション境界」（直前の開始 → 今回の停止）に該当する確定（finalized）セグメント列を渡す。具体的な境界の取り方（`TranscriptStore` のセッションスナップショット等）は /architect で確定。

> いずれの port も OS 型（`Translation.Session` / `NLLanguageRecognizer` / `FileHandle` 等）を引数・戻り値・throw 型に出さない。

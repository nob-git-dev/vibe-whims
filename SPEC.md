...
- [ ] 対象アプリ識別子・認識言語・出力先などの設定が config ファイルに分離されており、
      コードに直書きされていない。
- [ ] バンドル同梱の既定 config は、そのユーザーの Downloads 配下
      （`~/Downloads/speech-tap/transcript.txt`）をメイン保存先にする。
...
### 配布・インストール要件（M1 MacBook Air 向け）
- [ ] `scripts/make_app.sh` で arm64 専用の `build/SpeechTap.app` が生成され、Intel / x86_64 バイナリを含まない。
- [ ] `scripts/make_app.sh` で `/Applications` にインストール可能な `build/SpeechTap-arm64.pkg` が生成される。
- [ ] `build/SpeechTap-arm64.pkg` はインストール開始時に macOS 26.0 以上と Apple Silicon / arm64 を確認し、
      条件を満たさない Mac では `SpeechTap.app` をコピーする前にインストールを停止する。
- [ ] `scripts/make_app.sh` で `.app` を直接配布できる `build/SpeechTap-arm64.app.zip` が生成される。
- [ ] インストール後の `SpeechTap.app` は Finder でダブルクリックして起動できる。
...
### 録音中状態表示要件
- [ ] 文字化中（`SessionState.running`）はメニューバーのマイクアイコンが赤く表示され、録音中であることが視覚的に分かる。
- [ ] 停止・待機・エラーなど `running` 以外の状態では、メニューバーのマイクアイコンは通常表示に戻る。
...
- **必須 Apple API の availability 根拠**: 文字起こしに必須の `SpeechAnalyzer` / `SpeechTranscriber` は macOS 26.0+。
  音声取得の `AudioHardwareCreateProcessTap` は macOS 14.2+、`CATapDescription` は macOS 12.0+、
  現状スケルトンの Translation framework 基本 API は macOS 15.0+。
  したがって、配布時の実効下限は Speech 側に合わせて macOS 26.0+ とする。
- **3層アーキテクチャを厳守する**:
...
- **LLM 校正の明示 opt-in**: LLM による transcript 校正は翻訳機能とは別の任意機能とし、`LLM_CORRECTION_ENABLED=true` の設定がある場合だけ実行する。
  有効時はセッション確定テキストが設定された OpenAI 互換 API に送信され得るため、API base URL / model / API key は config ファイルで外部化し、既定は無効にする。
  無効時は従来通り音声・テキストを外部送信しない。
...
- **音声取得方式（アプリ別オーディオ取得の実現手段）** — **決定済み（→「## アーキテクチャ設計」ADR-1 参照）。**
  **採用: 候補A: Core Audio Process Tap**（`AudioHardwareCreateProcessTap` + `CATapDescription` + Aggregate Device, macOS 14.2+）。
...
[infrastructure]
      ProcessTapAudioSource ───▶ Core Audio Process Tap（macOS 14.2+。ADR-1。ADR-8: 関連プロセス群を集約しまとめてタップ＝非混入厳守）
...
**状況:**
仕様の最重要本質は「**対象アプリの音声のみを取得し、他アプリ・マイク・システム音を混入させない**」こと。実機は macOS 26.5 / Swift 6.3.2 / arm64、SpeechAnalyzer 利用可能。候補は (A) Core Audio Process Tap（`AudioHardwareCreateProcessTap` + `CATapDescription`、macOS 14.2+）と (B) ScreenCaptureKit（`SCStream` のアプリ別オーディオ、macOS 13+）。常駐メニューバーアプリであり、起動・操作の軽さ（UX）も重視する。
...
- **将来性**: macOS 14.2 以降の標準 Core Audio API で、システム音声キャプチャの「正式な道」として位置づけられている。

**理由（検証根拠 / 公式・準公式ソース）:**
- Apple「Capturing system audio with Core Audio taps」（`CATapDescription`/`AudioHardwareCreateProcessTap`/Aggregate Device、権限は `NSAudioCaptureUsageDescription`）。
...
## 2026-06-04: LLM transcript 校正機能

### 目的

停止時に作成済みのセッション transcript を、OpenAI 互換 API の LLM で校正し、音声認識特有の聞き間違い・誤変換・句読点不足・不要な反復を整えた読みやすい文章を別ファイルとして残す。

この機能は「原文保存」の既存本質を置き換えない。メインファイル `transcript.txt` と通常の Downloads セッション複本は、従来通り SpeechAnalyzer が返した原文のみを保存する。LLM 校正結果は `*-corrected.txt` の副産物としてのみ保存する。

### 固定要件との整合

- LLM 校正は `LLM_CORRECTION_ENABLED=true` の明示 opt-in の場合だけ実行する。
- 既定設定では LLM 校正を無効にし、既存のオンデバイス完結・外部送信なしの挙動を維持する。
- 有効時は transcript text が設定 API に送られるため、`LLM_API_BASE_URL` / `LLM_MODEL` / `LLM_API_KEY` / `LLM_TEMPERATURE` / `LLM_TIMEOUT_SECONDS` / `LLM_MAX_TOKENS` / `LLM_DISABLE_THINKING` を config で外部化する。
- OpenAI 互換 API 以外の SDK 依存は追加しない。HTTP/JSON は infrastructure に閉じる。
- domain は prompt・port・値型のみを持ち、URLSession / AppKit / OS API を import しない。

### アーキテクチャ設計

```text
presentation (SpeechTapApp)
  StopFlowCoordinator
    - 通常 Downloads 複本を書き出す
    - LLM 校正が有効なら TranscriptCorrector を呼び、CorrectedTranscriptExporter で corrected ファイルを書き出す
    - 校正失敗はユーザー通知のみ。メイン保存・通常複本・表示クリア判断は巻き戻さない

domain (SpeechTapDomain)
  TranscriptCorrectionPrompt
    - ASR 原文に忠実な校正プロンプトを構築する純粋値
  TranscriptCorrector
    - OpenAI 互換 API 等の校正器 port
  CorrectedTranscriptExporter
    - corrected ファイル書き出し port

infrastructure (SpeechTapInfrastructure)
  OpenAICompatibleTranscriptCorrector
    - /chat/completions へ system/user prompt を送る
    - Qwen3.5 などの thinking model では chat_template_kwargs で thinking を抑制できる
  DownloadsCorrectedTranscriptExporter
    - ~/Downloads/speech-tap-YYYYMMDD-HHmmss-corrected[-N].txt を上書きなしで作成
  ConfigLoader
    - LLM 校正設定を KEY=VALUE 形式から読む
```
...
- 意味を変えない。要約しない。翻訳しない。情報を足さない。
- 音声認識由来の誤字、誤変換、聞き間違い、句読点、段落、明らかな重複だけを直す。
- 固有名詞・専門用語・数式用語・時系列は文脈から明らかな場合だけ補正する。不確実なら原文を残す。
- 入力内で反復する同じ誤認識は、文脈から同一語と明らかな場合だけ一貫して補正する。
- 専門分野の定着語は明確な場合だけ正しい表記にする。数学史 transcript では「幾何学」「代数学」「論証数学」「十進位取り記数法」などが候補になる。
- 講義・会話の順序を維持する。見出しや箇条書きなど、原文にない構造を勝手に追加しない。
- 返答は校正済み本文のみ。思考過程、説明、差分、前置き、Markdown フェンスを出さない。

### 受け入れ条件

- [ ] `LLM_CORRECTION_ENABLED` が未指定または `false` の場合、LLM API は呼ばれず従来の保存挙動が維持される。
- [ ] `LLM_CORRECTION_ENABLED=true` かつ OpenAI 互換 API 設定が有効な場合、停止後に通常複本とは別に `speech-tap-YYYYMMDD-HHmmss-corrected.txt` が作成される。
- [ ] corrected ファイルの作成に失敗しても、メインファイルと通常 Downloads 複本は失われない。
- [ ] prompt は要約・翻訳・情報追加を禁止し、ASR 原文に忠実な校正だけを要求する。
- [ ] OpenAI 互換 API の base URL・model・API key は config で切り替えられる。
- [ ] Qwen3.5 など対応 endpoint 向けに `LLM_DISABLE_THINKING=true` を設定すると OpenAI 互換 JSON body のトップレベルに `chat_template_kwargs.enable_thinking=false` が送られる。未対応 endpoint では false のまま使える。
- [ ] 長文校正向けに `LLM_MAX_TOKENS` を設定すると OpenAI 互換 body の `max_tokens` として送られる。未指定なら送らない。
- [ ] domain は Foundation 以外の OS/UI/network framework を import しない。
- [ ] Downloads の長文 transcript を用いて、ローカル LLM の出力を Codex が原文対照で評価し、prompt を少なくとも 2 回改善する。

### prompt 評価ログ（2026-06-04）

Downloads の transcript を使ってローカル OpenAI 互換 API で評価を試行した。

- 入力候補:
  - `/Users/nobuyoshi/Downloads/speech-tap-20260603-175922.txt`（22,266 文字）
  - `/Users/nobuyoshi/Downloads/speech-tap-20260604-071740.txt`（12,361 文字）
  - `/Users/nobuyoshi/Downloads/speech-tap-20260601-233120.txt`（3,510 文字）
- API 確認:
  - `http://192.168.3.18:30000/v1/models` は `qwen3.5-122b` のみを返した。Qwen 2.5 は現時点で見つからなかった。
  - `http://192.168.3.18:11434`（Ollama）は接続不可だった。
- Docs / benchmark 確認:
  - `/Users/nobuyoshi/projects/hermes/SPEC-web-scraper-tool.md` の F14 は `qwen3.5-122b` を thinking model とし、litellm 呼び出し時に `extra_body={"chat_template_kwargs": {"enable_thinking": False}}` を必ず渡す、としている。これは SDK/litellm の引数であり、raw HTTP JSON では body に直接 merge される。
  - 同SPECには、修正前に `extra_body` を渡さないコードが 92.8 秒で timeout した実測が記録されている。
  - `/Users/nobuyoshi/projects/hermes/tools/web_scraper_tool/web_scraper_tool.py` も既定で `enable_thinking=false` の `extra_body` を構築して litellm に渡している。
  - `/Users/nobuyoshi/projects/dgx_spark_code/vllm-qwen122b-nvfp4/scripts/test-qwen122b.sh` は raw `/v1/chat/completions` のテスト payload で `extra_body.chat_template_kwargs.enable_thinking=false` を使っているが、今回の speech-tap 実測では raw HTTP の `extra_body` ネストは timeout し、top-level `chat_template_kwargs` で 300 文字チャンクが成功した。
  - `/Users/nobuyoshi/projects/benchmarks/japanese-llm-benchmark/rotorquant_benchmark.py` は日本語長文評価で `max_tokens=32768` / `timeout=300` を使い、`content` が空の場合に reasoning 系 field を見る処理を持つ。
- 試行結果:
  - 22,266 文字の full correction は header timeout。
  - 12,361 文字の streaming correction も 240 秒 timeout。
  - 3,510 文字でも現行モデルが reasoning 生成に入り、180 秒 timeout。
  - 短文でも `choices[].message.content == null` かつ `reasoning` のみ返るケースを確認。
  - vLLM 公式 docs（https://docs.vllm.ai/en/latest/serving/online_serving/openai_compatible_server/）は、OpenAI SDK では `extra_body` 引数を使い、HTTP を直接呼ぶ場合は extra parameter を JSON payload に直接 merge すると説明している。Chat API の extra parameter には `chat_template_kwargs` が含まれる。
  - speech-tap は Swift の raw HTTP client なので、`extra_body` ネストではなく top-level `chat_template_kwargs` を送る。
  - raw `extra_body` payload でも試行時点の `:30000` は timeout したが、Mission Control の environment 上は同サービスで `Running: 5 reqs` 相当の混雑があり、障害断定は避ける。
- prompt / 実装への反映:
  - **改善1**: prompt に「思考過程・校正方針・自己評価を出力しない」を追加。
  - **改善2**: 長文 transcript で反復していた専門語誤認識を踏まえ、「同じ誤認識の一貫補正」「専門分野の定着語は明確な場合だけ補正」を追加。
  - **改善3**: `LLM_DISABLE_THINKING=true` のとき OpenAI 互換 JSON body の top-level に `chat_template_kwargs.enable_thinking=false` を送る。
  - **改善4**: 長文校正で本文生成上限不足を避けるため `LLM_MAX_TOKENS` を追加し、Qwen3.5 の推奨値を 32768 とする。
  - **実装ガード**: OpenAI 互換 API の応答が reasoning-only / `content == null` の場合は corrected ファイルを作らず、校正失敗として扱う。校正本文として reasoning を保存しない。
  - **運用メモ**: 長文校正は endpoint の速度・thinking 制御に強く依存する。GX10 の `qwen3.5-122b` では `LLM_TIMEOUT_SECONDS=300` / `LLM_MAX_TOKENS=32768` / `LLM_DISABLE_THINKING=true` を推奨する。

### 300文字チャンク評価（2026-06-04）

`/Users/nobuyoshi/Downloads/speech-tap-20260603-175922.txt` から約300文字チャンクを3件抽出し、`qwen3.5-122b` に `chat_template_kwargs.enable_thinking=false` を top-level で送って評価した。

- API 結果:
  - 3/3 件で HTTP 200、`choices[].message.content` を取得。
  - 3/3 件で `reasoning == null`。reasoning-only 応答は発生しなかった。
  - elapsed: 16.5s / 17.3s / 15.8s、平均 16.5s。
  - prompt tokens: 546 / 567 / 550、completion tokens: 142 / 159 / 144。
- Codex judge による忠実度スコア:
  - chunk 1: 93%。「宇宙の幾化学」→「宇宙の幾何学」、「微分析文学」→「微分積分学」、「体制」→「体系化」などが妥当。末尾の「非ユークリッ」を「非ユークリッド」まで補完したが、語の途中補完として許容範囲。
  - chunk 2: 78%。「内角の輪」→「内角の和」、「何のことだかありませんね」→「何のことだかわかりませんね」は妥当。一方、冒頭の「公準の4番」相当を「第五公準」に変えており、原文の意味を変える過剰補正。
  - chunk 3: 80%。「制度」→「精度」、「レーザー光線など等」→「レーザー光線など」は妥当。一方、「二直角との差」を「二角和との差」に変えており、数学用語として過剰補正。
  - 平均忠実度: 84%。実用的なASR誤変換修正は多いが、数学用語・番号の過剰補正が2/3件に出たため、校正後の自動採用には judge ループまたは差分確認が必要。

### プロンプト改善ループ（2026-06-04）

同じ3チャンクを固定し、Codex judge で原文忠実度を比較した。`vllm:num_requests_running=5` の並走負荷があったため、応答時間は品質評価から除外し、本文の忠実度だけで採点した。

- baseline prompt:
  - 平均忠実度 84%。
  - 主な失点: 「公準の4番」相当を「第五公準」に変える番号推測、「二直角」を「二角和」にする数学用語の過剰補正、末尾の未完語補完。
- candidate 1:
  - 追加方針: 90%以上の確信がある箇所だけ修正、番号推測禁止、二直角など数学用語の言い換え禁止、チャンク末尾の補完禁止。
  - 結果: chunk 1 約98%、chunk 2 約91%、chunk 3 約97%、平均約95%。
  - 改善: 末尾「非ユークリッ」を補完しなくなり、「二直角」を保持した。
  - 残課題: chunk 2 で「公順」が一部残り、「行理」を「公準」に寄せる揺れが残った。
- candidate 2（採用）:
  - 追加方針: 音形に近い専門語候補を選ぶ。工順/高順/好順/公順 は公準、行理 は公理、行為 は文脈で明らかな場合だけ補正。同一チャンク内で同じ誤認識が反復する場合だけ一貫補正。
  - 結果: chunk 1 約98%、chunk 2 約97%、chunk 3 約97%、平均約97%。
  - 改善: 「公準の4番」「第5公準」を番号維持したまま修正し、「平行線行理」を「平行線公理」に補正。「二直角」は保持した。
  - 採用判断: baseline 84% → candidate 1 約95% → candidate 2 約97% と改善したため、candidate 2 を `TranscriptCorrectionPrompt.systemPrompt` に反映する。
